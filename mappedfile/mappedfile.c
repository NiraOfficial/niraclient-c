...
#else /* !_WIN32 && !HAVE_MMAP */
	// Added by Andy:
#	error "Support for either Posix mmap() or Windows CreateFileMappingA() is required!"
	// We don't want a fallback to malloc/fread in the fashion below, as it could
	// cause massive memory allocations.
	// Most implementations have mmap these days, so this shouldn't be a problem.
